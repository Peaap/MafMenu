...
local function IsValidFlashFrame(frame)
    return frame
        and IsValidNumber(frame.flashTimer)
        and IsValidNumber(frame.flashDuration)
        and IsValidNumber(frame.fadeInTime)
        and IsValidNumber(frame.fadeOutTime)
end
...
local function ScrubFlashFrames()
    ...
    for index = table.getn(FLASHFRAMES), 1, -1 do
        ...
        if not IsValidFlashFrame(frame) then
            ...
            DebugPrint("Removed invalid UIFrameFlash entry")
        end
    end
end

local function InstallFlashGuard()
    ...
    if UIFrameFlash then
        ...
        UIFrameFlash = function(frame, fadeInTime, fadeOutTime, flashDuration, showWhenDone, flashInHoldTime, flashOutHoldTime, syncId)
            if not frame or not IsValidNumber(fadeInTime) or not IsValidNumber(fadeOutTime) or not IsValidNumber(flashDuration) then
                DebugPrint("Blocked invalid UIFrameFlash call")
                return
            end
